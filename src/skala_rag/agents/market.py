"""시장성 근거 수집부. Rubric 판정/최종 market_analysis 생성은 다음 커밋 범위다."""
...
TECHNOLOGY_SEARCH_TERMS = {
    "KIVI": {"approach": "KV cache quantization", "paper_id": "2402.02750",
             "integration_targets": "Hugging Face Transformers vLLM"},
    "InfiniGen": {"approach": "KV cache offloading dynamic KV cache management",
                 "paper_id": "2406.19707", "integration_targets": "LLM serving GPU CPU offloading"},
}

# 기존 검색어/검색 캐시를 유지한다. 이번 수정의 핵심은 추가 검색이 아니라 선별과 검증이다.
SEARCH_QUERIES = {
    "market_size": (
...
TOPIC_RELATIONS = {
    ...
    "ecosystem": {"followup_research", "derived_implementation", "framework_influence", "implementation_update"},
}


def normalize_url(url: str) -> str:
    """중복 판정용: fragment와 추적용 인자만 제거하고 의미 있는 query는 유지한다."""
    # URL 경로는 대소문자를 구분하므로 lower()로 바꾸지 않는다.
    p = urlsplit(url)
...
def _is_original_paper(url: str, technology: str) -> bool:
    """ecosystem에서만 해당 기술의 원 논문 소개/미러 페이지를 제외한다."""
    p = urlsplit(url)
    host = (p.hostname or "").lower().removeprefix("www.")
    paper_id = re.escape(TECHNOLOGY_SEARCH_TERMS[technology]["paper_id"])
    if host in {"arxiv.org", "export.arxiv.org", "alphaxiv.org"}:
        return bool(re.fullmatch(rf"/(?:abs|pdf|html|overview)/{paper_id}(?:v\d+)?(?:\.pdf)?/?", p.path))
    if host == "huggingface.co" and re.fullmatch(rf"/papers/{paper_id}/?", p.path):
        return True
    # 실제 실패 사례에 포함된 KIVI 원 논문의 PMLR 소개 페이지.
    return technology == "KIVI" and host == "proceedings.mlr.press" and p.path.rstrip("/") == "/v235/liu24bz.html"


def is_search_result_relevant(result: dict, technology: str, topic: MarketTopic) -> bool:
    """비용이 적은 후보 필터. 통과한 것만으로 유효한 근거가 되지는 않는다."""
    ...
    try:
        ...
        return False
    text = _normalized_text(" ".join(_text(result.get(k)) for k in ("title", "content", "url")))
    if topic == "market_size":
        # 관련 시장도 허용하지만 모든 페이지를 무조건 통과시키지는 않는다.
        domain = any(t in text for t in ("llm", "inference", "artificial intelligence", "ai infrastructure", "인공지능", "추론"))
        market = any(t in text for t in ("market", "cagr", "growth", "revenue", "forecast", "시장", "성장률"))
...
def _error(stage: str, topic: str, exc: Exception, **context: Any) -> dict:
    # SDK 오류 내용에 키나 원문이 포함될 수 있어 일반 로그에는 종류와 위치만 남긴다.
    return {"stage": stage, "topic": topic, "error_type": type(exc).__name__,
            "error": str(exc) if isinstance(exc, EvidenceValidationError) else f"{stage} 단계 실패 ({type(exc).__name__})", **context}
...
def collect_market_evidence(
    ...
    *,
    ...
    """상한을 둔 수집. direct만 evidence에 넣고 간접 근거, 제외 기록, 오류는 분리한다."""
    ...
    if type(max_attempts_per_topic) is not int or not sources_per_topic <= max_attempts_per_topic <= 30:
        raise ValueError("max_attempts_per_topic는 sources_per_topic 이상, 30 이하여야 합니다")

    output: dict[str, Any] = {
        "technology": technology, "evidence": [], "indirect_evidence": [],
        "skipped": [], "errors": [], "searches": [], "topic_status": {},
    }
    ...
    for topic in dict.fromkeys(topics):
        ...
        for template in SEARCH_QUERIES[topic]:
            ...
            candidates = []
            try:
                hits = get_search_results(query, max_results=max_results, mode=mode, cache_dir=cache_dir)
                ...
                for rank, hit in enumerate(hits, start=1):
                    if is_search_result_relevant(hit, technology, topic):
                        ...
                        output["skipped"].append({"stage": "candidate_filter", "topic": topic,
                            "url": hit.get("url"), "reason": "irrelevant_or_original_paper"})
                output["searches"].append({"topic": topic, "query": query,
                    "returned": len(hits), "relevant": len(candidates)})
        ...
        for query, rank, hit in ordered:
            ...
            try:
                ...
                if not is_search_result_relevant({"url": source.get("url", url),
                        "title": source.get("title"), "content": raw}, technology, topic):
                    output["skipped"].append({"stage": "source_filter", "topic": topic,
                        "url": url, "reason": "source_not_relevant"})
                    continue
                digest = sha256(raw.encode("utf-8")).hexdigest()
                if digest in seen_contents:
                    output["skipped"].append({"stage": "content_dedupe", "topic": topic,
                        "url": url, "reason": "duplicate_content"})
                    continue
                stage = "summarize"
                summary = summarize_source(source, question=question, topic=topic, mode=mode, cache_dir=cache_dir)
                seen_contents.add(digest)
                status = summary["evidence_status"]
                if status == "not_found":
                    output["skipped"].append({"stage": "evidence_filter", "topic": topic,
                        "url": url, "reason": "not_found", "detail": summary["reason"]})
                    continue
                ...
                evidence_key = f"{source_id}|{technology}|{topic}|{summary['quote']}|{summary['claim']}"
                entry = {**summary, "technology": technology, "topic": topic,
                    "source_id": source_id, "evidence_id": "web-evidence-" + sha256(evidence_key.encode()).hexdigest()[:24],
                    "claim_type": "reported_fact", "search_query": query,
                    "search_rank": rank, "search_score": hit.get("score")}
                # 원문에 직접 적힌 사실도 해당 평가 항목에는 간접 근거일 수 있다.
                if status == "indirect" or summary["relation_kind"] not in TOPIC_RELATIONS[topic]:
                    entry["evidence_status"] = "indirect"
                    output["indirect_evidence"].append(entry)
                    continue
                if topic == "market_size" and (
                    summary["scope"] == "unspecified" or not re.search(r"\d", summary["quote"])
                ):
                    output["skipped"].append({"stage": "evidence_filter", "topic": topic,
                        "url": url, "reason": "market_scope_or_numeric_evidence_missing"})
                    continue
        ...
        output["topic_status"][topic] = {
            ...
            "accepted": accepted, "attempted": attempted, "candidate_count": len(ordered),
            "error_count": len(output["errors"]) - topic_error_start,
            "attempt_limit_reached": attempted >= max_attempts_per_topic and accepted < sources_per_topic,
        }

    output["missing_topics"] = [t for t, info in output["topic_status"].items() if not info["accepted"]]
    # 최종 Rubric의 complete가 아니다. 자료 수집 단계의 상태만 나타낸다.
    output["status"] = "insufficient_evidence" if output["missing_topics"] else "has_evidence"
    return output
