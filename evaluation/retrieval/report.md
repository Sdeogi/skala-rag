...
|---|---|---|---|---|---|
| kivi-01 | KIVI | principle | O | 0.33 | KIVI는 Key 캐시와 Value 캐시를 각각 어떤 축으로 양자화하는가? |
| kivi-02 | KIVI | principle | O | 0.33 | KIVI가 KV 캐시를 몇 비트로 압축하는가? |
| kivi-03 | KIVI | experimental_setup | O | 0.20 | KIVI 실험에 사용된 모델은 어떤 것들인가? |
| kivi-04 | KIVI | experimental_setup | O | 0.33 | KIVI 성능 평가에 사용된 벤치마크나 태스크는 무엇인가? |
| kivi-05 | KIVI | performance | O | 0.25 | KIVI를 적용하면 메모리 사용량과 처리량(throughput)이 얼마나 개선되는가? |
...
| infinigen-01 | InfiniGen | principle | O | 0.25 | InfiniGen은 어떤 방식으로 중요한 KV를 미리 예측하고 가져오는가? |
...
| infinigen-05 | InfiniGen | performance | O | 1.00 | InfiniGen은 기존 방법 대비 몇 배의 속도 향상을 보이는가? |
